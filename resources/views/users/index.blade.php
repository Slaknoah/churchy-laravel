          ...
          <div class="h-100 layers">
            @can('create', App\User::class)
              <div class="p-20 bgc-grey-100 layer w-100">
                <a href="/users/create" class="btn btn-danger btn-block">New user</a>
              </div>
            @endcan
                                    ...
                                    @if(count($users) > 0)
                                        @foreach($users as $key=>$user)
                                            <tr>
                                                ...
                                                @php
                                                  $rolesString = '';
                                                  foreach ($user->roles()->get(['name']) as $key => $role) {
                                                    $rolesString .= $key === 0?$role->name : ', ' . $role->name;
                                                  }                                                    
                                                @endphp
                                                <td>{{$rolesString}}</td>
                                                <td>
                                                    {!! Form::open(['action' => ['UserController@destroy', $user->id], 'method' => 'POST']) !!}
                                                        ...
                                                        <button onclick="return confirm('Are you sure you want to delete?')" class="btn btn-danger c-white-500 @cannot('delete', $user) disabled @endcannot" href="" type="submit">
